...
%==============================================================================
    properties (Access = protected)
        ...
        eqnIdx = 0;       % which row does this contribution occupy in the
                          % output vectors
        rhsDummyNode = IrNodePotentialFlow.empty; % empty if nullEqn==false,
        % otherwise points to the IR tree node that contains the RHS dummy IO.

        accessIdx = 0;
        impContribIdx = 0;
        impEqnDependPfVec = MsPotential.empty;
        impEqnDependVarVec = MsVariable.empty;
        nodeCollapse = false;
    end

    methods

        function obj = IrNodeContribution(lhsNode, rhsNode)
        ...
        function setImplicitEqnDependPfVec(thisContrib, pfVec)
        % SETIMPLICITEQNDEPENDPFVEC
            thisContrib.impEqnDependPfVec = pfVec;
        end

        function setImplicitEqnDependVarVec(thisContrib, varVec)
        % SETIMPLICITEQNDEPENDVARVEC
            thisContrib.impEqnDependVarVec = varVec;
        end

        function pfVec = getImplicitEqnDependPfVec(thisContrib)
        % GETIMPLICITEQNDEPENDPFVEC
            pfVec = thisContrib.impEqnDependPfVec;
        end

        function varVec = getImplicitEqnDependVarVec(thisContrib)
        % GETIMPLICITEQNDEPENDVARVEC
            varVec = thisContrib.impEqnDependVarVec;
        end
    ...
    methods (Access = protected)
        ...
        function [headNode, generateSub] = generateDerivative(thisContrib, derivObj)
            ...
            if thisContrib.implicit == false
                ...
                lhsVarNodeF = IrNodeVariable(lhsDerivLabelF);
                rhsVarNodeF = IrNodeVariable(lhsDerivLabelF);
                lhsVarNodeQ = IrNodeVariable(lhsDerivLabelQ);
                rhsVarNodeQ = IrNodeVariable(lhsDerivLabelQ);
                ...
                lhsVarNodeF = IrNodeVariable(lhsDerivLabelF);
                rhsVarNodeF = IrNodeNumericalNull();
                lhsVarNodeQ = IrNodeVariable(lhsDerivLabelQ);
                rhsVarNodeQ = IrNodeNumericalNull();
            ...
            if rhsDerivTreeF.isNull() == false
                if inverse == false
                    rhsDerivTreeF = IrNodeNumerical.getSummOfNodes(rhsVarNodeF, ...
                                                                 rhsDerivTreeF);
                else
                    rhsDerivTreeF = IrNodeNumerical.getDifferenceOfNodes(rhsVarNodeF, ...
                                                                 rhsDerivTreeF);
                end
            % add the derivative trees to the RHSs of assignment nodes
                assignNodeF = IrNodeAssignment(lhsVarNodeF, rhsDerivTreeF);
            ...
            if rhsDerivTreeQ.isNull() == false
                if inverse == false
                    rhsDerivTreeQ = IrNodeNumerical.getSummOfNodes(rhsVarNodeQ, ...
                                                                 rhsDerivTreeQ);
                else
                    rhsDerivTreeQ = IrNodeNumerical.getDifferenceOfNodes(rhsVarNodeQ, ...
                                                                 rhsDerivTreeQ);
                end

                assignNodeQ = IrNodeAssignment(lhsVarNodeQ, rhsDerivTreeQ);
