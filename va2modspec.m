...
%==============================================================================
    ...
    inPar = inputParser();
    inPar.addRequired('inFileName', @ischar);
    ...
    % Octave specific operations
    % are we in Octave
    inOctave = (exist('OCTAVE_VERSION', 'builtin') > 0);

    if inOctave == true
        copyWarn = warning('query', 'Octave:classdef-to-struct');
        if strcmp(copyWarn.state, 'on')
            warning('off', 'Octave:classdef-to-struct');
        end

        pathWarn = warning('query', 'Octave:data-file-in-path');
        if strcmp(pathWarn.state, 'on')
    ...
    % to them.
    derPfVec = module.getInputPfVecC();
