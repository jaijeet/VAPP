...
%==============================================================================
    ...
    % nature, discipline.
    nature_map = LookupTableVapp(); % values: natures -> keys: access strings
    access_map = LookupTableVapp(); % values: access strings -> keys nature labels
    discipline_map = LookupTableVapp();
...
% units, access, ddt_nature, idt_nature, abstol
    ...
    while true
        ...
        if is_keyword(tok, 'access')
            ...
            if isempty(tok) == false
                ...
                access_map.addValue(nature_name, access_label);
            end
    ...
    if isempty(access_label) == false
        nature_map.addValue(access_label, nature);
    end
...
% PARSE_DISCIPLINE
    ...
    while true
        ...
        if is_keyword(tok, 'potential')
            ...
            if isempty(tok) == false
                ...
                try 
                    access_label = access_map.getValue(nature_name);
                    nature_struct = nature_map.getValue(access_label);
    ...
    if isempty(discipline_name) == false
        discipline_map.addValue(discipline_name, discipline);
    end
end
...
function [AST, new_next_pos] = ...
    ...
    access_arr = {};
    discipline_arr = discipline_map.getKeyArr();
    while true
        ...
        elseif is_one_of_the_identifiers(tok, discipline_arr, str_utils)
            discipline_name = tok.value;
            discipline = discipline_map.getValue(discipline_name);
